...
- Treat `test_model_1.xlsx`, `test_model_cleaned.parquet`, `test_fuel_cleaned.parquet`, and `test_calculation.xlsx` as upstream inputs — do not read from `refer/` directly.
...
- **Stage 3 (`build_analyst.py`)**: reads data from `test_model_1.xlsx` only. Copies `test_calculation.xlsx` as the output base; appends new analyst sheets; replaces `Data` sheet with fresh sorted data (no `Powertrain`). Does NOT read from `refer/` model file.
