...
| 3 | Decompose `build_cleaned.main()` | **PARTIAL** | `load_reference_maps`, `add_derived_columns`, `rolling_merge`, `resolve_bev_review_records` extracted — but `main()` is still 191 lines (target ~30). Still inline: master-model glob/discovery, existing-parquet load+validation, both parquet saves, output-file copy/overwrite prompt, `pipeline_state.json` write, `known_models.txt` tracking |
